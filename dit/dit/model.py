...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        # We use a simple learnable initialization here for simplicity, 
        # but in production, fixed sin-cos is often preferred.
    ...
    def forward(self, x, t, y):
        ...
        """
        x = self.x_embedder(x) # (N, Hidden, H/P, W/P)
        x = x.flatten(2).transpose(1, 2) # (N, L, Hidden)
        x = x + self.pos_embed
        
        t = self.t_embedder(t)                   # (N, Hidden)
        y = self.y_embedder(y)                   # (N, Hidden)
        c = t + y                                # (N, Hidden)
        
        for block in self.blocks:
            x = block(x, c)
            
        x = self.final_layer(x, c)               # (N, L, Patch*Patch*Out_C)
        x = self.unpatchify(x)                   # (N, Out_C, H, W)
        return x
